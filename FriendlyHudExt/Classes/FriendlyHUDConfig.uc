...
var config bool IgnoreSentinel;
...
var config Color AmmoFullColor;
var config Color AmmoEmptyColor;
...
var config bool AvatarIconEnabled;
var config float AvatarIconSize;
var config float AvatarIconGap;
var config float AvatarIconOffsetY;
...
function Initialized()
{
    ...
    else
    {
        if (INIVersion < 2)
        {
            ...
            SortStrategy = 0;
            SelfSortStrategy = 1;
            DynamicOpacity = class'FriendlyHUDExt.FriendlyHUDHelper'.static.MakeCubicInterpCurve(1.f, 1.f, 4.f, 0.f);
            Opacity = 1.f;
            IconSize = 32.f;
            IconOffset = 0.f;
            IconGap = 4.f;
            ...
            OldBGColor = class'FriendlyHUDExt.FriendlyHUDHelper'.static.ColorFromString(BGColor);
            ...
            NameColor = class'FriendlyHUDExt.FriendlyHUDHelper'.static.ColorFromString(TextColor);
...
exec function ResetFHUDBar()
{
    ...
    FriendIconGap = 2.f;
    FriendIconOffsetY = 0.f;
    AvatarIconEnabled = true;
    AvatarIconSize = 24.f;
    AvatarIconGap = 2.f;
    AvatarIconOffsetY = 0.f;
    NameMarginX = 0.f;
    NameMarginY = 0.f;
    NameScale = 1.f;
    BarWidthMin = 122.f;
...
exec function ResetFHUDColors()
{
    ...
    AmmoFullColor = MakeColor(20, 175, 20, 255);
    AmmoEmptyColor = MakeColor(175, 0, 0, 255);
...
exec function PrintFHUDHelp()
{
    ...
    ConsolePrint("SetFHUDAmmoFullColor <byte R> <byte G> <byte B> <byte A = 192>: controls the color of the bar when the sentinels ammo is full (default is 20,175,20,255)");
    ConsolePrint("SetFHUDAmmoEmptyColor <byte R> <byte G> <byte B> <byte A = 192>: controls the color of the bar when the sentinels ammo is empty (default is 175,0,0,255)");
    ...
    ConsolePrint("SetFHUDAvatarIconEnabled <bool>: controls whether an player avatar icon should appear next to the player name");
    ConsolePrint("SetFHUDAvatrIconSize <float>: controls the size of the player avatar icon (default is 24)");
    ConsolePrint("SetFHUDAvatarIconGap <float>: controls the gap between the player avatar and the player name (default is 4)");
    ConsolePrint("SetFHUDAvatarIconOffsetY <float>: controls the Y alignment offset of the player avatar icon (default is 0)");
...
exec function SetFHUDArmorBlockProportions(float Width, optional string Ratios)
{
    ...
    for (I = 0; I < Params.Length; I++)
    {
        ...
        if (Params.Length > 1)
        {
            ...
            if (Ratio < class'FriendlyHUDExt.FriendlyHUDInteraction'.const.FLOAT_EPSILON) continue;
...
{
    ArmorBlockOutline = class'FriendlyHUDExt.FriendlyHUDHelper'.static.MakeOutline(Top, Right, Bottom, Left);
    SaveAndUpdate();
}
...
{
    HealthBlockOutline = class'FriendlyHUDExt.FriendlyHUDHelper'.static.MakeOutline(Top, Right, Bottom, Left);
    SaveAndUpdate();
}
...
exec function SetFHUDAvatarIconEnabled(bool Value)
{
    AvatarIconEnabled = Value;
    SaveAndUpdate();
}

exec function SetFHUDAvatarIconSize(float Value)
{
    AvatarIconSize = FMax(Value, 0.f);
    SaveAndUpdate();
}

exec function SetFHUDAvatarIconGap(float Value)
{
    AvatarIconGap = Value;
    SaveAndUpdate();
}

exec function SetFHUDAvatarIconOffsetY(float Value)
{
    AvatarIconOffsetY = Value;
    SaveAndUpdate();
}
...
{
    DynamicOpacity = class'FriendlyHUDExt.FriendlyHUDHelper'.static.MakeCubicInterpCurve(P0, P1, T0, T1);
    SaveAndUpdate();
}
...
exec function SetFHUDAmmoFullColor(byte R, byte G, byte B, optional byte A = 192)
{
    AmmoFullColor = MakeColor(R, G, B, A);
    SaveAndUpdate();
}

exec function SetFHUDAmmoEmptyColor(byte R, byte G, byte B, optional byte A = 192)
{
    AmmoEmptyColor = MakeColor(R, G, B, A);
    SaveAndUpdate();
}
