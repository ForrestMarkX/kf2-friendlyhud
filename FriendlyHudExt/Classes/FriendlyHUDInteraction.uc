...
struct UI_RuntimeVars
{
    ...
    var float AvatarIconSize, AvatarIconGap, AvatarIconOffsetY;
...
// Mod additions
var int CurrentVoiceCommsHighlightAlpha, CurrentAlphaDelta;
...
function FriendlyHUDReplicationLink GetRepLink()
{
    ...
    foreach KFPlayerOwner.WorldInfo.DynamicActors(class'FriendlyHUDExt.FriendlyHUDReplicationLink', RepLink)
    {
        return RepLink;
    }
...
function UpdateNames()
{
    ...
    while (RepInfo != None)
    {
        for (I = 0; I < class'FriendlyHUDExt.FriendlyHUDReplicationInfo'.const.REP_INFO_COUNT; I++)
        {
...
event PostRender(Canvas Canvas)
{
    ...
    // Allow modders to force the HUD off
    if( ForceDisableHUD(self, Canvas) ) return;
...
function UpdateRuntimeVars(optional Canvas Canvas)
{
    ...
    R.ResScale = class'FriendlyHUDExt.FriendlyHUDHelper'.static.GetResolutionScale(Canvas);
    ...
    R.AvatarIconSize = HUDConfig.AvatarIconSize * R.Scale;
    R.AvatarIconGap = HUDConfig.AvatarIconGap * R.Scale;
    R.AvatarIconOffsetY = HUDConfig.AvatarIconOffsetY * R.Scale;
...
function bool IsPRIRenderable(FriendlyHUDReplicationInfo RepInfo, int RepIndex)
{
    ...
    // Don't render if Sentinel drawing is disabled or the Sentinel is not owned by the local player
    SRI = SentinelReplicationInfo(KFPRI);
    if( SRI != None && HUDConfig.IgnoreSentinel && SRI.TurretOwner != None && SRI.TurretOwner.Instigator != None && SRI.TurretOwner.Instigator.PlayerReplicationInfo == KFPlayerOwner.PlayerReplicationInfo ) return true;
...
function bool DrawHealthBarItem(Canvas Canvas, const out PlayerItemInfo ItemInfo, float PosX, float PosY)
{
    ...
    if( HUDConfig.AvatarIconEnabled )
    {
        if( KFPRI.Avatar != None )
        {
            AvatarIconPosX = PlayerNamePosX;
            AvatarIconPosY = PosY + R.LineHeight - R.AvatarIconSize + R.AvatarIconOffsetY;
        
            SetCanvasColor(Canvas, HUDConfig.ShadowColor);
            Canvas.SetPos(AvatarIconPosX, AvatarIconPosY + 1);
            Canvas.DrawTile(KFPRI.Avatar, R.AvatarIconSize, R.AvatarIconSize, 0, 0, KFPRI.Avatar.SizeX, KFPRI.Avatar.SizeY);
            
            SetCanvasColor(Canvas, HUD.WhiteColor);
            Canvas.SetPos(AvatarIconPosX, AvatarIconPosY);
            Canvas.DrawTile(KFPRI.Avatar, R.AvatarIconSize, R.AvatarIconSize, 0, 0, KFPRI.Avatar.SizeX, KFPRI.Avatar.SizeY);
            
            PlayerNamePosX += R.AvatarIconSize + R.AvatarIconGap;
        }
        else
        {
    ...
    if (ManualModeActive)
    {
        ...
        {
            class'FriendlyHUDExt.FriendlyHUDHelper'.static.DrawSelection(
                Canvas,
                SelectionPosX,
                SelectionPosY,
                SelectionWidth,
...
)
{
    ...
    switch( BarType )
    {
        ...
        case BT_Ammo:
            ...
            BarColor.R = Approach(HUDConfig.AmmoEmptyColor.R, HUDConfig.AmmoFullColor.R, Abs(HUDConfig.AmmoEmptyColor.R - HUDConfig.AmmoFullColor.R) * BarRatio);
            BarColor.G = Approach(HUDConfig.AmmoEmptyColor.G, HUDConfig.AmmoFullColor.G, Abs(HUDConfig.AmmoEmptyColor.G - HUDConfig.AmmoFullColor.G) * BarRatio);
            BarColor.B = Approach(HUDConfig.AmmoEmptyColor.B, HUDConfig.AmmoFullColor.B, Abs(HUDConfig.AmmoEmptyColor.B - HUDConfig.AmmoFullColor.B) * BarRatio);
    ...
    for (I = 0; I < BlockCount; I++)
    {
        ...
        if (HUDConfig.DrawDebugRatios)
        {
            DebugRatioText = class'FriendlyHUDExt.FriendlyHUDHelper'.static.FloatToString(P1 * BlockRatio, 2) $ "/" $ class'FriendlyHUDExt.FriendlyHUDHelper'.static.FloatToString(BlockRatio, 2);
...
// Modding delegates ahead
delegate Texture2D GetPrestigeIcon(FriendlyHUDInteraction FHUDInfo, HUD HUDInterface, KFPlayerReplicationInfo KFPRI, byte PrestigeLevel)
{
	return KFPRI.CurrentPerkClass.default.PrestigeIcons[PrestigeLevel - 1];
}

delegate PreDrawPlayerItem(FriendlyHUDInteraction FHUDInfo, Canvas Canvas, KFPlayerReplicationInfo KFPRI, HUD HUDInterface)
{
	return;
}

delegate bool OverridePlayerNameDraw(FriendlyHUDInteraction FHUDInfo, KFPlayerReplicationInfo KFPRI, HUD HUDInterface, Canvas Canvas, float PlayerNamePosX, float PlayerNamePosY, string PlayerName, FontRenderInfo TextFontRenderInfo, const PlayerItemInfo ItemInfo, optional byte IsFriend)
{
	return false;
}

delegate bool GetPlayerIsChatting(FriendlyHUDInteraction FHUDInfo, KFPlayerReplicationInfo KFPRI, HUD HUDInterface)
{
	return false;
}

delegate DrawPlayerChatIcon(FriendlyHUDInteraction FHUDInfo, KFPlayerReplicationInfo KFPRI, HUD HUDInterface, Canvas Canvas, float ChatIconPosX, float ChatIconPosY, out float PlayerNamePosX)
{
	return;
}

delegate Font GetCanvasFont(FriendlyHUDInteraction FHUDInfo, HUD HUDInterface)
{
	return class'KFGameEngine'.static.GetKFCanvasFont();
}

delegate float GetCanvasFontScale(FriendlyHUDInteraction FHUDInfo, HUD HUDInterface)
{
	return class'KFGameEngine'.static.GetKFFontScale();
}

delegate bool ForceDisableHUD(FriendlyHUDInteraction FHUDInfo, Canvas Canvas)
{
	return false;
}
